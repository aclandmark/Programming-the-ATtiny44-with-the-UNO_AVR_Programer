...
{long error;
	...
	setup_Attiny_HW;
	OCR1C = 0xFF;														//Defines top for Timer 1
	...
	{Timer_T0_sub(T0_delay_5ms);
		OSCCAL = OSCCAL_WV + m;
		error = compute_error(5,1);										//Signed error values
		if ((error < -1000) ||
...
ISR (PCINT0_vect){												//Pin change interrupt on DI pin
	...
	if(Transmitter_active){										//USI in default state (transmitter ready but not in use): Calculate OSCCAL errors
		if (!(TCCR1B)) {
			TCNT1_sum = 0;
			TCNT1 = 0;
			TCCR1B = 0x44;										//1MHz clock plus reset pre-scaler
		}
	...
	ISR(TIMER1_OVF_vect){TCNT1_sum += 0x100;}
