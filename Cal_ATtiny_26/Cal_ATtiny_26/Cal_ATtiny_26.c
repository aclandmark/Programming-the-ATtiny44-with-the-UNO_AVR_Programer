						...
						/************************************************************************************************/
						ISR(TIMER0_OVF0_vect)												//Clock signal for USI: shifts data in the USIDataRegister
						{
							if(Transmitter_active)TCNT0 = Tx_clock;
							else TCNT0 = Rx_clock;											//Necessary because receiver initially sets half the baud rate
						TCCR0 |= 1 << PSR0;		//TCNT0 = 0;														//Reset T0
						}
							...
							if(Transmitter_active){										//USI in default state (transmitter ready but not in use): Calculate OSCCAL errors
								...
								error_counter +=1;}}
									...
									{TCNT0 = 0;
										...
										TIFR = (1 << TOV0);									//Clear spurious interrupts
										
										TIMSK |= (1 << TOIE0);								//Enable interrupt on output compare
